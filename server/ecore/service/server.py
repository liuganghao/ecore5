...
from ecore.release import nt_service_name
import ecore.tools.config as config
from ecore.tools import stripped_sys_argv, dumpstacks, log_ormcache_stats
...
class RequestHandler(werkzeug.serving.WSGIRequestHandler):
    def setup(self):
        ...
        me.name = 'ecore.service.http.request.%s' % (me.ident,)
...
class FSWatcher(object):
    def __init__(self):
        ...
        for path in ecore.modules.module.ad_paths:
...
class ThreadedServer(CommonServer):
    ...
    def cron_thread(self, number):
        while True:
            ...
            registries = ecore.modules.registry.RegistryManager.registries
            _logger.debug('cron%d polling for jobs', number)
            for db_name, registry in registries.iteritems():
                while registry.ready:
                    acquired = ecore.addons.base.ir.ir_cron.ir_cron._acquire_job(db_name)
                    if not acquired:
                        break
    ...
    def cron_spawn(self):
        ...
        for i in range(ecore.tools.config['max_cron_threads']):
            def target():
                self.cron_thread(i)
            t = threading.Thread(target=target, name="ecore.service.cron.cron%d" % i)
            t.setDaemon(True)
            t.start()
    ...
    def http_spawn(self):
        t = threading.Thread(target=self.http_thread, name="ecore.service.httpd")
        t.setDaemon(True)
        t.start()
    ...
    def stop(self):
        ...
        _logger.debug('--')
        ecore.modules.registry.RegistryManager.delete_all()
        logging.shutdown()
...
class PreforkServer(CommonServer):
    ...
    def long_polling_spawn(self):
        ...
        cmd = nargs[0]
        cmd = os.path.join(os.path.dirname(cmd), "ecore-gevent")
        nargs[0] = cmd
...
class Worker(object):
    ...
    def setproctitle(self, title=""):
        setproctitle('ecore: %s %s %s' % (self.__class__.__name__, self.pid, title))
    ...
    def run(self):
        try:
            ...
            _logger.info("Worker (%s) exiting. request_count: %s, registry count: %s.",
                         ...
                         len(ecore.modules.registry.RegistryManager.registries))
            self.stop()
        except Exception:
...
class WorkerCron(Worker):
    ...
    def _db_list(self):
        if config['db_name']:
            ...
            db_names = ecore.service.db.list_dbs(True)
        return db_names

    def process_work(self):
        rpc_request = logging.getLogger('ecore.netsvc.rpc.request')
        ...
        if len(db_names):
            ...
            import ecore.addons.base as base
...
def load_server_wide_modules():
    for m in ecore.conf.server_wide_modules:
        try:
            ecore.modules.module.load_ecore_module(m)
        except Exception:
            msg = ''
            if m == 'web':
                msg = """
The `web` module is provided by the addons found in the `ecore-web` project.
Maybe you forgot to add those addons in your addons_path configuration."""
            _logger.exception('Failed to load server-wide module `%s`.%s', m, msg)

def _reexec(updated_modules=None):
    """reexecute ecore-server process with (nearly) the same arguments"""
    if ecore.tools.osutil.is_running_as_nt_service():
...
def load_test_file_yml(registry, test_file):
    with registry.cursor() as cr:
        ecore.tools.convert_yaml_import(cr, 'base', file(test_file), 'test', {}, 'init')
    ...
    for mod_name, mod_mod in sys.modules.items():
        if mod_mod:
            ...
            if test_path == mod_path:
                ...
                stream = ecore.modules.module.TestStream()
...
def start(preload=None, stop=False):
    """ Start the ecore http server and cron processor.
    """
    global server
    load_server_wide_modules()
    if ecore.evented:
        server = GeventServer(ecore.service.wsgi_server.application)
    elif config['workers']:
        server = PreforkServer(ecore.service.wsgi_server.application)
    else:
        server = ThreadedServer(ecore.service.wsgi_server.application)
    ...
    if getattr(ecore, 'phoenix', False):
        if watcher:
            watcher.stop()
        _reexec()
