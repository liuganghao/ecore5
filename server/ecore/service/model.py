...
import ecore
from ecore.exceptions import UserError, ValidationError, QWebException
from ecore.tools.translate import translate
...
def check(f):
    ...
    def wrapper(___dbname, *args, **kwargs):
                ...
                try:
                    if args and len(args) > 1:
                        ...
                        if len(args) > 3 and isinstance(args[3], (long, int, list)):
                            ...
                            ids = []
                    cr = ecore.sql_db.db_connect(dbname).cursor()
        ...
        while True:
            try:
                if ecore.registry(dbname)._init and not ecore.tools.config['test_enable']:
                    raise ecore.exceptions.Warning('Currently, this database is not fully loaded and can not be used.')
                ...
                registry = ecore.registry(dbname)
...
def execute_cr(cr, uid, obj, method, *args, **kw):
    object = ecore.registry(cr.dbname).get(obj)
    if object is None:
...
def execute(db, uid, obj, method, *args, **kw):
    ...
    with ecore.registry(db).cursor() as cr:
