...
# User code must use the exceptions defined in ``ecore.exceptions`` (not
...
def xmlrpc_return(start_response, service, method, params, string_faultcode=False):
    ...
    """
    # Map eCore core exceptions to XML-RPC fault codes. Specific exceptions
    # defined in ``ecore.exceptions`` are mapped to specific fault codes;
    ...
    # exception handling.
    try:
        result = ecore.http.dispatch_rpc(service, method, params)
...
def xmlrpc_handle_exception_int(e):
    if isinstance(e, ecore.exceptions.UserError):
        fault = xmlrpclib.Fault(RPC_FAULT_CODE_WARNING, ecore.tools.ustr(e.value))
        ...
        #formatted_info = ecore.tools.exception_to_unicode(e) + '\n' + info
...
def application_unproxied(environ, start_response):
    ...
    # web.session.eCoreSession.send() and at RPC dispatch in
    # ecore.service.web_services.objects_proxy.dispatch().
    ...
    with ecore.api.Environment.manage():
        ...
        for handler in [wsgi_xmlrpc, ecore.http.root]:
