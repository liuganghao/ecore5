...
ecore.testing.start_services = function () {
    var factories = ecore.__DEBUG__.factories;
  ...
  return ecore.process_jobs(jobs, services);
};

ecore.testing.MockRPC = function (session) {
    this.responses = {};
};

ecore.testing.MockRPC.prototype.clear = function () {
    this.responses = {};
};

ecore.testing.MockRPC.prototype.interceptRPC = function (session) {
    session.rpc = this.rpc.bind(this);
};

ecore.testing.MockRPC.prototype.add = function (spec, handler, no_override) {
...
ecore.testing.MockRPC.prototype.rpc =  function (url, rparams, options) {
    if (_.isString(url)) {
        url = {url: url};
    }
    var fn, params;
...
ecore.testing.noop = function () {};

ecore.define_section = function (name, section_deps) {
    ...
    function test () { 
        ...
        QUnit.test(name, function (assert) {
            var services = ecore.testing.start_services();
            var deps = _.map(section_deps.concat(dep_names), function (name) { return services[name]; });
            services.qweb.add_template(ecore.testing.templates);
            mock.clear();
    ...
    var mock = new ecore.testing.MockRPC();
