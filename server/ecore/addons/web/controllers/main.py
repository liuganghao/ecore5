...
from ecore.addons.base.ir.ir_qweb import AssetsBundle, QWebTemplateNotFound
from ecore.modules import get_module_path, get_resource_path
from ecore.tools import topological_sort
from ecore.tools.translate import _
from ecore.tools import ustr
from ecore import http
import mimetypes
from ecore.http import request, serialize_exception as _serialize_exception, STATIC_CACHE
from ecore.exceptions import AccessError
...
if hasattr(sys, 'frozen'):
    ...
    loader = jinja2.PackageLoader('ecore.addons.web', "views")
    ...
    try:
        ...
        with registry.cursor() as cr:
            return module_installed(
                environment=Environment(cr, ecore.SUPERUSER_ID, {}))
    except Exception:
        pass
    return {}

def module_boot(db=None):
    server_wide_modules = ecore.conf.server_wide_modules or ['web']
    serverside = []
    dbside = []
...
def manifest_list(extension, mods=None, db=None, debug=None):
    ...
    if debug is not None:
        _logger.warning("ecore.addons.web.main.manifest_list(): debug parameter is deprecated")
...
def fix_view_modes(action):
    """ For historical reasons, eCore has weird dealings in relation to
...
def _local_web_translations(trans_file):
    ...
    try:
        ...
        return
    for x in po:
        if x.id and x.string and "ecore-web" in x.auto_comments:
...
def db_info():
    version_info = ecore.service.common.exp_version()
    return {
...
class Home(http.Controller):
    ...
    def web_login(self, redirect=None, **kw):
        ...
        if not request.uid:
            request.uid = ecore.SUPERUSER_ID
...
class WebClient(http.Controller):
    ...
    def translations(self, mods=None, lang=None):
        ...
        messages = ir_translation.search_read(request.cr, uid, [('module','in',mods),('lang','=',lang),
                                               ('comments','like','ecore-web'),('value','!=',False),
                                               ('value','!=','')],
    ...
    def version_info(self):
        return ecore.service.common.exp_version()
...
class Proxy(http.Controller):
    ...
    def post(self, path):
        ...
        with request.session.load_request_data() as data:
            ...
            headers = {'X-Ecore-Session-Id': request.session.sid}
...
class Database(http.Controller):
    ...
    def _render_template(self, **d):
        ...
        d['insecure'] = ecore.tools.config['admin_passwd'] == 'admin'
        d['list_db'] = ecore.tools.config['list_db']
        d['langs'] = ecore.service.db.exp_list_lang()
        # databases list
        d['databases'] = []
        try:
    ...
    def backup(self, master_pwd, name, backup_format = 'zip'):
        try:
            ecore.service.db.check_super(master_pwd)
            ...
            dump_stream = ecore.service.db.dump_db(name, None, backup_format)
...
class Session(http.Controller):
    ...
    def modules(self):
        ...
        return module_installed(environment=request.env(user=ecore.SUPERUSER_ID))
...
class Binary(http.Controller):
    ...
    def content_image(self, xmlid=None, model='ir.attachment', id=None, field='datas', filename_field='datas_fname', unique=None, filename=None, mimetype=None, download=None, width=0, height=0):
        ...
        if content and width and height:
            ...
            if height > 500:
                height = 500
            content = ecore.tools.image_resize_image(base64_source=content, size=(width, height), encoding='base64', filetype='PNG')
    ...
    def company_logo(self, dbname=None, **kw):
        ...
        if not dbname:
            ...
            try:
                ...
                registry = ecore.modules.registry.Registry(dbname)
                with registry.cursor() as cr:
...
class Export(http.Controller):
    ...
    def get_fields(self, model, prefix='', parent_name= '',
        ...
        fields_sequence = sorted(fields.iteritems(),
            key=lambda field: ecore.tools.ustr(field[1].get('string', '')))
...
class ExportFormat(object):
    ...
    def from_data(self, fields, rows):
        """ Conversion method from eCore's export data to whatever the
