...
Client actions are the client-side version of eCore's "Server
...
In the eCore Web code, a client action only requires two pieces of
information:
...
* Providing said object. Two different types of objects can be mapped
  ...
  * An eCore Web widget, which must inherit from
    :js:class:`ecore.web.Widget`
...
* if, on the other hand, the client action maps to a
  :js:class:`~ecore.web.Widget`, that
  :js:class:`~ecore.web.Widget` will be instantiated and added to
  the web client's canvas, with the usual
  :js:class:`~ecore.web.Widget` lifecycle (essentially, it will
...
object::
    ...
    // Registers the object 'ecore.web_dashboard.Widget' to the client
...
At this point, the generic :js:class:`~ecore.web.Widget` lifecycle
...
should be performed in the :js:func:`~ecore.web.Widget.start()`
method.

.. note::

    As required by :js:class:`~ecore.web.Widget`'s contract, if
    :js:func:`~ecore.web.Widget.start()` executes any asynchronous
