...
example in eCore is dashboards), queries should be restricted to a
...
eCore Web's :js:class:`~ecore.web.Widget` has an attribute
providing its DOM root :js:attr:`Widget.$el <ecore.web.Widget.$el>`,
...
* All interactive components (components displaying information to the
  ...
  :class:`~ecore.web.Widget` and correctly implement and use its API
  and lifecycle.
