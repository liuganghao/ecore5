...
Building an eCore Web module
==============================

There is no significant distinction between an eCore Web module and
an eCore module, the web part is mostly additional data and code
inside a regular eCore module. This allows providing more seamless
features by integrating your module deeper into the web client.

A Basic Module
--------------

A very basic eCore module structure will be our starting point:
...
This is a sufficient minimal declaration of a valid eCore module.

Web Declaration
---------------

There is no such thing as a "web module" declaration. An eCore
...
.. note::
    ...
    restart the eCore server itself for it to be taken in account.
...
provides: an instance of the eCore Web client, with the contents of
...
its :js:func:`~ecore.web.Widget.start` to add some content to its
DOM:
...
will require restarting the eCore Server to see the changes, as
usual):
...
.. note::
    ...
    Prefixing CSS rules with both ``.ecore`` (to ensure the rule
    will apply only within the confines of the eCore Web client) and
...
be done in :js:func:`~ecore.web.Widget.start` but that gets unwieldy
...
eCore Web's template language is :doc:`qweb`. Although any
...
integration to eCore Web widgets.
...
To prevent this irremediable loss, we can use eCore's support for
...
:js:func:`~ecore.base.Widget.start()` is to perform *asynchronous*
...
recorded previously using the :js:class:`~ecore.web.Query` interface
...
4. And finally — after restarting eCore — navigating to the test
...
side-effect of this is to :js:func:`~ecore.web.Widget.start` it,
...
:js:func:`set a widget's DOM <ecore.web.Widget.setElement>`, let's
