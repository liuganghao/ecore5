...
class ImBus(models.Model):
    ...
    def sendmany(self, notifications):
        ...
        if channels:
            ...
            def notify():
                with ecore.sql_db.db_connect('postgres').cursor() as cr:
...
class ImDispatch(object):
    ...
    def poll(self, dbname, channels, last, options=None, timeout=TIMEOUT):
        ...
        if not ecore.evented:
            ...
            current.setName("ecore.longpolling.request.%s" % current.ident)

        registry = ecore.registry(dbname)

        # immediatly returns if past notifications exist
        with registry.cursor() as cr:
            notifications = registry['bus.bus'].poll(cr, ecore.SUPERUSER_ID, channels, last, options)
        ...
        if not notifications:
            ...
            try:
                ...
                with registry.cursor() as cr:
                    notifications = registry['bus.bus'].poll(cr, ecore.SUPERUSER_ID, channels, last, options, force_status=True)
            except Exception:
                # timeout
                pass
