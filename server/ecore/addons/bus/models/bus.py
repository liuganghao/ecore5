...
class ImBus(models.Model):
    ...
    def sendmany(self, notifications):
        ...
        if channels:
            ...
            if not ecore.tools.config['test_enable']:
                self._cr.commit()
            with ecore.sql_db.db_connect('postgres').cursor() as cr2:
...
class ImDispatch(object):
    ...
    def poll(self, dbname, channels, last, timeout=TIMEOUT):
        ...
        with registry.cursor() as cr:
            notifications = registry['bus.bus'].poll(cr, ecore.SUPERUSER_ID, channels, last)
