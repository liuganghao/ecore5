...
from ecore.osv.orm import browse_record
import ecore.report.interface
from ecore.report.report_sxw import report_sxw, report_rml
from ecore.tools import ormcache
from ecore.tools.safe_eval import safe_eval as eval
from ecore.tools.translate import _
import ecore.workflow
from ecore.exceptions import MissingError, UserError
...
class ir_actions_report_xml(osv.osv):
    ...
    def _lookup_report(self, cr, name):
        ...
        if 'report.' + name in ecore.report.interface.report_int._reports:
            new_report = ecore.report.interface.report_int._reports['report.' + name]
        else:
            ...
            if r:
                if r['report_type'] in ['qweb-pdf', 'qweb-html']:
                    ...
                    if r['parser']:
                        kwargs = { 'parser': operator.attrgetter(r['parser'])(ecore.addons) }
                    else:
                        kwargs = {}
    ...
    def render_report(self, cr, uid, res_ids, name, data, context=None):
        ...
        if isinstance(new_report, (str, unicode)):  # Qweb report
            ...
            if ecore.tools.config['test_enable'] and not tools.config['test_report_directory']:
...
class ir_actions_act_window(osv.osv):
    ...
    def exists(self):
        ...
        if len(existing) < len(self):
            ...
            (self - existing)._cache.update(ecore.fields.FailedValue(exc))
        return existing
...
class ir_actions_server(osv.osv):
    ...
    Since eCore 8.0 a button 'Create Menu Action' button is available on the
...
# To return an action, assign: action = {...}""",
    ...
    def run_action_trigger(self, cr, uid, action, eval_context=None, context=None):
        ...
        if action.use_relational_model == 'relational':
            ...
            if not isinstance(record, ecore.models.BaseModel):
    ...
    def _get_eval_context(self, cr, uid, action=None, context=None):
        ...
        env = ecore.api.Environment(cr, uid, context)
        ...
        eval_context.update({
            ...
            'workflow': workflow,
            # Exceptions
            'Warning': ecore.exceptions.Warning,
            # record
