...
from ecore import models, tools, api
from ecore.modules.registry import RegistryManager
from ecore.osv import fields, osv
from ecore.osv.orm import BaseModel, Model, MAGIC_COLUMNS
from ecore.exceptions import UserError, AccessError
from ecore.tools import config
...
class ir_model(osv.osv):
    ...
    def unlink(self, cr, user, ids, context=None):
        ...
        if not context.get(MODULE_UNINSTALL_FLAG):
            ...
            # reload is done independently in ecore.modules.loading
    ...
    def write(self, cr, user, ids, vals, context=None):
        ...
        # Filter out operations 4 link from field id, because ecore-web
...
class ir_model_constraint(Model):
    """
    This model tracks PostgreSQL foreign keys and constraints used by eCore
    models.
    """
    ...
    def _module_data_uninstall(self, cr, uid, ids, context=None):
        ...
        for data in self.browse(cr, uid, ids, context):
            ...
            name = ecore.tools.ustr(data.name)
            typ = data.type
...
class ir_model_relation(Model):
    """
    This model tracks PostgreSQL tables used to implement eCore many2many
    relations.
    """
...
class ir_model_access(osv.osv):
    ...
    def check(self, cr, uid, model, mode='read', raise_exception=True, context=None):
        ...
        if not r and raise_exception:
            ...
            raise ecore.exceptions.AccessError(msg % msg_params)
        return bool(r)
...
class ir_model_data(osv.osv):
    """Holds external identifier keys for records in the database.
       This has two main uses:
           ...
           * allows tracking the origin of data installed by eCore
    ...
    def ir_set(self, cr, uid, key, key2, name, models, value, replace=True, isobject=False, meta=None, xml_id=False):
        ...
        res = cr.fetchone()
        ir_values_obj = ecore.registry(cr.dbname)['ir.values']
        if not res:
    ...
    def _module_data_uninstall(self, cr, uid, modules_to_remove, context=None):
        ...
        for model,res_id in wkf_todo:
            try:
                ecore.workflow.trg_write(uid, model, res_id, cr)
            except Exception:
            ...
            for model, res_id in to_unlink:
                ...
                if model == 'ir.model.fields':
                    ...
                    if not field.exists():
                        ...
                        continue
                    if field.name in ecore.models.LOG_ACCESS_COLUMNS and self.pool[field.model]._log_access:
                        continue
                    if field.name == 'id':
                        continue
