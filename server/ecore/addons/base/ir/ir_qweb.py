...
import ecore.tools.lru
from ecore.exceptions import QWebException
from ecore.fields import Datetime
from ecore.http import request
from ecore.tools.safe_eval import safe_eval as eval
from ecore.osv import osv, orm, fields
from ecore.tools import html_escape as escape
from ecore.tools.misc import find_in_path
from ecore.tools.translate import _
from ecore.modules.module import get_resource_path
    ...
    * alternatively, override :meth:`~.get_converter_for` and return an
    ...
    def render_node(self, element, qwebcontext, generated_attributes=''):
        ...
        if debugger is not None:
            if ecore.tools.config['dev_mode']:
    ...
    def get_converter_for(self, field_type):
        """ returns a :class:`~ecore.models.Model` used to render a
        ``t-field``.
    ...
    def value_to_html(self, cr, uid, value, field, options=None, context=None):
        ...
        if isinstance(value, basestring):
            value = datetime.datetime.strptime(
                value[:10], ecore.tools.DEFAULT_SERVER_DATE_FORMAT)

        if options and 'format' in options:
            pattern = options['format']
        else:
            strftime_pattern = lang.date_format
            pattern = ecore.tools.posix_to_ldml(strftime_pattern, locale=locale)
...
class DateTimeConverter(osv.AbstractModel):
    ...
    def value_to_html(self, cr, uid, value, field, options=None, context=None):
        ...
        if isinstance(value, basestring):
            value = datetime.datetime.strptime(
                value, ecore.tools.DEFAULT_SERVER_DATETIME_FORMAT)
...
class RelativeDatetimeConverter(osv.AbstractModel):
    ...
    def value_to_html(self, cr, uid, value, field, options=None, context=None):
        parse_format = ecore.tools.DEFAULT_SERVER_DATETIME_FORMAT
        locale = babel.Locale.parse(
            self.user_lang(cr, uid, context=context).code)
...
class AssetsBundle(object):
    ...
    def clean_attachments(self, type):
        ...
        attachment_ids = ira.search(self.cr, ecore.SUPERUSER_ID, domain, context=self.context)
        return ira.unlink(self.cr, ecore.SUPERUSER_ID, attachment_ids, context=self.context)
    ...
    def get_attachments(self, type, inc=None):
        ...
        attachment_ids = ira.search(self.cr, ecore.SUPERUSER_ID, domain, order='name asc', context=self.context)
        return ira.browse(self.cr, ecore.SUPERUSER_ID, attachment_ids, context=self.context)
    ...
    def save_attachment(self, type, content, inc=None):
        ...
        values["public"] = True
        values["type"] = 'binary'
        attachment_id = ira.create(self.cr, ecore.SUPERUSER_ID, values, context=self.context)
        url = '/web/content/%s-%s/%s' % (attachment_id, self.version, values["datas_fname"])
        values["name"] = url
        values["url"] = url
        values["datas"] = content.encode('utf8').encode('base64')

        ira.write(self.cr, ecore.SUPERUSER_ID, attachment_id, values, context=self.context)
        ...
        return ira.browse(self.cr, ecore.SUPERUSER_ID, attachment_id, context=self.context)
    ...
    def stat(self):
        if not (self.inline or self._filename or self._ir_attach):
            ...
            try:
                ...
                ira = self.registry['ir.attachment']
                attach = ira.search_read(self.cr, ecore.SUPERUSER_ID, domain, fields, context=self.context)
        ...
        if self.url:
            try:
                ...
                with self.cr.savepoint():
                    ira_id = ira.search(self.cr, ecore.SUPERUSER_ID, domain, context=self.context)
                    datas = self.content.encode('utf8').encode('base64')
                    if ira_id:
                        # TODO: update only if needed
                        ira.write(self.cr, ecore.SUPERUSER_ID, ira_id, {'datas': datas},
                                  context=self.context)
                    else:
                        ira.create(self.cr, ecore.SUPERUSER_ID, dict(
                            datas=datas,
                            mimetype='text/css',
                            type='binary',
                            name=url,
