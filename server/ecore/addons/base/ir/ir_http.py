...
class ir_http(osv.AbstractModel):
    ...
    def _auth_method_public(self):
        if not request.session.uid:
            dummy, request.uid = self.pool['ir.model.data'].get_object_reference(request.cr, ecore.SUPERUSER_ID, 'base', 'public_user')
        else:
            request.uid = request.session.uid
    ...
    def _authenticate(self, auth_method='user'):
        try:
            if request.session.uid:
                ...
                except (ecore.exceptions.AccessDenied, ecore.http.SessionExpiredException):
                    # All other exceptions mean undetermined status (e.g. connection pool full),
                    # let them bubble up
                    request.session.logout(keep_db=True)
            if request.uid is None:
                getattr(self, "_auth_method_%s" % auth_method)()
        except (ecore.exceptions.AccessDenied, ecore.http.SessionExpiredException, werkzeug.exceptions.HTTPException):
            raise
        except Exception:
    ...
    def _serve_attachment(self):
        ...
        attach = self.pool['ir.attachment'].search_read(request.cr, ecore.SUPERUSER_ID, domain, ['__last_update', 'datas', 'name', 'mimetype', 'checksum'], context=request.context)
        if attach:
    ...
    def _postprocess_args(self, arguments, rule):
        ...
        for name, arg in arguments.items():
            if isinstance(arg, orm.browse_record) and arg._uid is UID_PLACEHOLDER:
                ...
                try:
                    arg.exists()
                except ecore.models.MissingError:
    ...
    def routing_map(self):
        if not hasattr(self, '_routing_map'):
            ...
            if ecore.tools.config['test_enable']:
                installed.add(ecore.modules.module.current_test)
            mods = [''] + ecore.conf.server_wide_modules + sorted(installed)
