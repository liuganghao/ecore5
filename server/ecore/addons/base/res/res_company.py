    ...
    def _get_address_data(self, cr, uid, ids, field_names, arg, context=None):
        ...
        for company in self.browse(cr, uid, ids, context=context):
            ...
            if company.partner_id:
                address_data = part_obj.address_get(cr, ecore.SUPERUSER_ID, [company.partner_id.id], adr_pref=['contact'])
                if address_data['contact']:
                    address = part_obj.read(cr, ecore.SUPERUSER_ID, [address_data['contact']], field_names, context=context)[0]
