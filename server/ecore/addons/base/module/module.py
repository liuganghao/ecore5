...
import ecore
import ecore.exceptions
from ecore import modules, tools
from ecore.modules.db import create_categories
from ecore.modules import get_module_resource
from ecore.tools import ormcache
from ecore.tools.parse_version import parse_version
from ecore.tools.translate import _
from ecore.tools import html_sanitize
from ecore.osv import osv, orm, fields
from ecore import api, fields as fields2
...
class module(osv.osv):
    ...
    def _button_immediate_function(self, cr, uid, ids, function, context=None):
        ...
        registry = ecore.modules.registry.RegistryManager.new(cr.dbname, update_module=True)
    ...
    def install_from_urls(self, cr, uid, urls, context=None):
        ...
        ECORE = ecore.release.product_name.lower()
        ...
        try:
            ...
            for module_name, url in urls.items():
                ...
                try:
                    _logger.info('Downloading module `%s` from eCore Apps', module_name)
            ...
            for module_name, url in urls.items():
                if module_name == ECORE or not url:
                    continue    # ECORE is special case, handled below, and no URL means local module
            ...
            if urls.get(ECORE, None):
                ...
                # copy all modules in the SERVER/ecore/addons directory to the new "ecore" module (except base itself)
                for d in os.listdir(base_path):
                    if d != 'base' and os.path.isdir(os.path.join(base_path, d)):
                        destdir = os.path.join(tmp, ECORE, 'addons', d)    # XXX 'ecore' subdirectory ?
                        shutil.copytree(os.path.join(base_path, d), destdir)

                # then replace the server by the new "base" module
                server_dir = ecore.tools.config['root_path']      # XXX or dirname()
                bck = backup(server_dir)
                _logger.info('Copy downloaded module `ecore` to `%s`', server_dir)
                shutil.move(os.path.join(tmp, ECORE), server_dir)
                #if bck:
                #    shutil.rmtree(bck)
    ...
    def get_apps_server(self, cr, uid, context=None):
        return tools.config.get('apps_server', 'https://apps.ecore.cool/apps')
