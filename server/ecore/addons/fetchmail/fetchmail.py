...
from ecore import tools, api, SUPERUSER_ID
...
class fetchmail_server(osv.osv):
    ...
    _defaults = {
        ...
        'priority': 5,
        'attach': True,
        'script': '/mail/static/scripts/ecore_mailgate.py',
    }
...
ecore_mailgate.py --host=HOSTNAME --port=PORT -u %(uid)d -p PASSWORD -d %(dbname)s

Example configuration for the postfix mta running locally:

/etc/postfix/virtual_aliases:
@youdomain ecore_mailgate@localhost

/etc/aliases:
ecore_mailgate: "|/path/to/ecore-mailgate.py --host=localhost -u %(uid)d -p PASSWORD -d %(dbname)s"
