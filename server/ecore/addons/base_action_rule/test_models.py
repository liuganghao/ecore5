...
class lead_test(osv.Model):
    ...
    customer = ecore.fields.Boolean(related='partner_id.customer', readonly=True, store=True)
...
class line_test(osv.Model):
    ...
    name = ecore.fields.Char()
    lead_id = ecore.fields.Many2one('base.action.rule.lead.test', ondelete='cascade')
    user_id = ecore.fields.Many2one('res.users')
