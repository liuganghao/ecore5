...
class meeting_invitation(http.Controller):
    ...
    def accept(self, db, token, action, id, **kwargs):
        ...
        with registry.cursor() as cr:
            attendee_id = attendee_pool.search(cr, ecore.SUPERUSER_ID, [('access_token', '=', token), ('state', '!=', 'accepted')])
            if attendee_id:
                attendee_pool.do_accept(cr, ecore.SUPERUSER_ID, attendee_id)
    ...
    def declined(self, db, token, action, id):
        ...
        attendee_pool = registry.get('calendar.attendee')
        with registry.cursor() as cr:
            attendee_id = attendee_pool.search(cr, ecore.SUPERUSER_ID, [('access_token', '=', token), ('state', '!=', 'declined')])
            if attendee_id:
                attendee_pool.do_decline(cr, ecore.SUPERUSER_ID, attendee_id)
        return self.view(db, token, action, id, view='form')
