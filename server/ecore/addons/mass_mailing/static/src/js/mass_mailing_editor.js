ecore.define('mass_mailing.editor', function (require) {
"use strict";
...
var _set_value = window.top.ecore[callback+"_updown"];
var ecore_top = window.top.ecore;
window.top.ecore[callback+"_updown"] = function (value, fields_values, field_name) {
    if (!window) {
        delete ecore_top[callback+"_updown"];
        return;
    }
    ...
    if (fields_values.mailing_model && web_editor.editor_bar) {
        ...
        if (value.indexOf('on_change_model_and_list') !== -1) {
            ecore_top[callback+"_downup"](_val);
        }
    }
};
