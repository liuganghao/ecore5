...
from ecore.tools import assertion_report, lazy_property, classproperty, config, topological_sort
from ecore.tools.lru import LRU
...
class Registry(Mapping):
    ...
    def __init__(self, db_name):
        ...
        self._db = ecore.sql_db.db_connect(db_name)
        ...
        # Inter-process signaling (used only when ecore.multi_process is True):
        ...
        cr = self.cursor()
        has_unaccent = ecore.modules.db.has_unaccent(cr)
        if ecore.tools.config['unaccent'] and not has_unaccent:
            _logger.warning("The option --unaccent was given but no unaccent() function was found in database.")
        self.has_unaccent = ecore.tools.config['unaccent'] and has_unaccent
        cr.close()
...
class RegistryManager(object):
        ...
        with cls.lock():
            with ecore.api.Environment.manage():
                ...
                try:
                    ...
                    ecore.modules.load_modules(registry._db, force_demo, status, update_module)
                except Exception:
    ...
    def check_registry_signaling(cls, db_name):
        ...
        """
        changed = False
        if ecore.multi_process and db_name in cls.registries:
