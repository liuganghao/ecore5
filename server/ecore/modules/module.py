...
class AddonsImportHook(object):
    """
    Import hook to load eCore addons from multiple paths.

    eCore implements its own import-hook to load its addons. eCore
    ...
    are living in `ecore.addons`. The good way to import such modules is
    thus `import ecore.addons.module`.
    """

    def find_module(self, module_name, package_path=None):
        module_parts = module_name.split('.')
        if len(module_parts) == 3 and module_name.startswith('ecore.addons.'):
    ...
    def load_module(self, module_name):
        ...
        mod = imp.load_module('ecore.addons.' + module_part, f, path, descr)
        sys.modules['ecore.addons.' + module_part] = mod
        return mod

def initialize_sys_path():
    """
    Setup an import-hook to be able to import eCore addons from the different
    addons paths.

    This ensures something like ``import crm`` (or even
    ``import ecore.addons.crm``) works even if the addons are not in the
    PYTHONPATH.
    """
    global ad_paths
    global hooked
...
def get_module_filetree(module, dir='.'):
    ...
    files = ecore.tools.osutil.listdir(path, True)
...
def init_module_models(cr, module_name, obj_list):
    ...
    for t in todo:
        t[1](cr, *t[2])
    if obj_list:
        obj_list[0].recompute(cr, ecore.SUPERUSER_ID, {})
    cr.commit()

def load_ecore_module(module_name):
    """ Load an eCore module, if not already loaded.
    ...
    try:
        ...
        __import__('ecore.addons.' + module_name)
        ...
        if info['post_load']:
            getattr(sys.modules['ecore.addons.' + module_name], info['post_load'])()
...
class TestStream(object):
    def __init__(self, logger_name='ecore.tests'):
...
def run_unit_tests(module_name, dbname, position=runs_at_install):
    ...
    for m in mods:
        ...
        if suite.countTestCases():
            ...
            if time.time() - t0 > 5:
                _logger.log(25, "%s tested in %.2fs, %s queries", m.__name__, time.time() - t0, ecore.sql_db.sql_counter - t0_sql)
