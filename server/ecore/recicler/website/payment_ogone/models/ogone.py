...
from ecore.addons.payment.models.payment_acquirer import ValidationError
from ecore.addons.payment_ogone.controllers.main import OgoneController
from ecore.addons.payment_ogone.data import ogone
from ecore.osv import osv, fields
from ecore.tools import float_round, DEFAULT_SERVER_DATE_FORMAT
from ecore.tools.float_utils import float_compare, float_repr
...
class PaymentAcquirerOgone(osv.Model):
    ...
    def _ogone_generate_shasign(self, acquirer, inout, values):
        ...
        :param string inout: 'in' (ecore contacting ogone) or 'out' (ogone
                             contacting ecore). In this last case only some
...
class PaymentTxOgone(osv.Model):
    ...
    def ogone_s2s_do_transaction(self, cr, uid, id, context=None, **kwargs):
        ...
        reference = tx.reference or "eCore-%s-%s" % (datetime.datetime.now().strftime('%y%m%d_%H%M%S'), tx.partner_id.id)
...
class PaymentMethod(osv.Model):
    ...
    def ogone_create(self, cr, uid, values, context=None):
        if values.get('cc_number'):
            ...
            alias = 'eCore-NEW-ALIAS-%s' % time.time()
