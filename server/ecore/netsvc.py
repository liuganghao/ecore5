...
def LocalService(name):
    """
    The ecore.netsvc.LocalService() function is deprecated. It still works
    in two cases: workflows and reports. For workflows, instead of using
    LocalService('workflow'), ecore.workflow should be used (better yet,
    methods on ecore.osv.orm.Model should be used). For reports,
    ecore.report.render_report() should be used (methods on the Model should
    be provided too in the future).
    """
    assert ecore.conf.deprecation.allow_local_service
    _logger.warning("LocalService() is deprecated since march 2013 (it was called with '%s')." % name)

    if name == 'workflow':
        return ecore.workflow

    if name.startswith('report.'):
        report = ecore.report.interface.report_int._reports.get(name)
        if report:
            return report
        else:
...
class PostgreSQLHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with tools.ignore(Exception), tools.mute_logger('ecore.sql_db'), sql_db.db_connect(dbname, allow_uri=True).cursor() as cr:
            cr.autocommit(True)
...
def init_logger():
    ...
    # Check that handler.stream has a fileno() method: when running eCore
...
DEFAULT_LOG_CONFIGURATION = [
    'ecore.workflow.workitem:WARNING',
    'ecore.http.rpc.request:INFO',
    'ecore.http.rpc.response:INFO',
    'ecore.addons.web.http:INFO',
    'ecore.sql_db:INFO',
    ':INFO',
]
PSEUDOCONFIG_MAPPER = {
    'debug_rpc_answer': ['ecore:DEBUG','ecore.http.rpc.request:DEBUG', 'ecore.http.rpc.response:DEBUG'],
    'debug_rpc': ['ecore:DEBUG','ecore.http.rpc.request:DEBUG'],
    'debug': ['ecore:DEBUG'],
    'debug_sql': ['ecore.sql_db:DEBUG'],
    'info': [],
    'warn': ['ecore:WARNING', 'werkzeug:WARNING'],
    'error': ['ecore:ERROR', 'werkzeug:ERROR'],
    'critical': ['ecore:CRITICAL', 'werkzeug:CRITICAL'],
}
