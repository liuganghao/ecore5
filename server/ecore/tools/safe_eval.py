...
eCore domain strings, conditions and expressions, mostly based on locals
...
def safe_eval(expr, globals_dict=None, locals_dict=None, mode="eval", nocopy=False, locals_builtins=False):
    ...
    an eCore domain expression from an untrusted source.
    ...
    except ecore.exceptions.except_orm:
        raise
    except ecore.exceptions.Warning:
        raise
    except ecore.exceptions.RedirectWarning:
        raise
    except ecore.exceptions.AccessDenied:
        raise
    except ecore.exceptions.AccessError:
        raise
    except OperationalError:
        # Do not hide PostgreSQL low-level exceptions, to let the auto-replay
        # of serialized transactions work its magic
        raise
    except ecore.exceptions.MissingError:
        raise
    except Exception, e:
        import sys
        exc_info = sys.exc_info()
