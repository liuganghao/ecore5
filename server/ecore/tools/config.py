#ecore.loggers.handlers. -*- coding: utf-8 -*-
...
import ecore
import ecore.conf
import ecore.loglevels as loglevels
import logging
import ecore.release as release
import appdirs
...
def _deduplicate_loggers(loggers):
    ...
    if for some odd reason people use :option`ecore.py --save`` all the time.
    """
...
class configmanager(object):
    def __init__(self, fname=None):
        ...
        group.add_option("-s", "--save", action="store_true", dest="save", default=False,
                          help="save configuration to ~/.ecore_serverrc")
        ...
        group.add_option("-D", "--data-dir", dest="data_dir", my_default=_get_default_datadir(),
                         help="Directory where to store eCore data")
        ...
        group.add_option('--log-handler', action="append", default=[], my_default=DEFAULT_LOG_HANDLER, metavar="PREFIX:LEVEL", help='setup a handler at LEVEL for a given PREFIX. An empty PREFIX indicates the root logger. This option can be repeated. Example: "ecore.orm:DEBUG" or "werkzeug:CRITICAL" (default: ":INFO")')
        group.add_option('--log-request', action="append_const", dest="log_handler", const="ecore.http.rpc.request:DEBUG", help='shortcut for --log-handler=ecore.http.rpc.request:DEBUG')
        group.add_option('--log-response', action="append_const", dest="log_handler", const="ecore.http.rpc.response:DEBUG", help='shortcut for --log-handler=ecore.http.rpc.response:DEBUG')
        group.add_option('--log-web', action="append_const", dest="log_handler", const="ecore.http:DEBUG", help='shortcut for --log-handler=ecore.http:DEBUG')
        group.add_option('--log-sql', action="append_const", dest="log_handler", const="ecore.sql_db:DEBUG", help='shortcut for --log-handler=ecore.sql_db:DEBUG')
        ...
        group = optparse.OptionGroup(parser, "Internationalisation options",
            "Use these options to translate eCore to another language."
    ...
    def parse_config(self, args=None):
        ...
        This method initializes ecore.tools.config and ecore.conf (the
        ...
        Typical usage of this method:
            ...
            ecore.tools.config.parse_config(sys.argv[1:])
        """
        self._parse_config(args)
        ecore.netsvc.init_logger()
        ecore.modules.module.initialize_sys_path()
    ...
    def _parse_config(self, args=None):
        ...
        # TODO use appdirs
        if os.name == 'nt':
            rcfilepath = os.path.join(os.path.abspath(os.path.dirname(sys.argv[0])), 'ecore-server.conf')
        else:
        ...
        self.options['root_path'] = os.path.abspath(os.path.expanduser(os.path.expandvars(os.path.dirname(ecore.__file__))))
        ...
        ecore.conf.addons_paths = self.options['addons_path'].split(',')
        if opt.server_wide_modules:
            ecore.conf.server_wide_modules = map(lambda m: m.strip(), opt.server_wide_modules.split(','))
        else:
            ecore.conf.server_wide_modules = ['web','web_kanban']
    ...
    def _is_addons_path(self, path):
        for f in os.listdir(path):
            ...
            if os.path.isdir(modpath):
                ...
                if hasfile('__init__.py') and (hasfile('__ecore__.py') or hasfile('__terp__.py')):
                    return True
        return False
