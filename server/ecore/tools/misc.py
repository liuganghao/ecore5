...
"""
Miscellaneous tools used by eCore.
"""
...
from ecore.loglevels import get_encodings, ustr, exception_to_unicode     # noqa
...
def exec_pg_environ():
    ...
    """
    env = os.environ.copy()
    if ecore.tools.config['db_host']:
        env['PGHOST'] = ecore.tools.config['db_host']
    if ecore.tools.config['db_port']:
        env['PGPORT'] = str(ecore.tools.config['db_port'])
    if ecore.tools.config['db_user']:
        env['PGUSER'] = ecore.tools.config['db_user']
    if ecore.tools.config['db_password']:
        env['PGPASSWORD'] = ecore.tools.config['db_password']
    return env
...
def file_open(name, mode="r", subdir='addons', pathinfo=False):
    """Open a file from the eCore root, using a subdir folder.
    ...
    """
    import ecore.modules as addons
    ...
    if os.path.isabs(name):
        ...
        for root in adps + [rtp]:
            ...
            # It is outside the eCore root: skip zipfile lookup.
...
def scan_languages():
    """ Returns all languages supported by eCore for translation
...
class upload_data_thread(threading.Thread):
    ...
    def run(self):
        try:
            ...
            fp = urllib.urlopen('http://www.ecore.cool/scripts/survey.php', args)
            fp.read()
            fp.close()
        except Exception:
            pass
...
class UnquoteEvalContext(defaultdict):
    """Defaultdict-based evaluation context that returns 
       ...
       the evaluation.
       Mostly useful for evaluating eCore domains/contexts that
...
class mute_logger(object):
    ...
    Can be used as context manager or decorator.
        ...
        @mute_logger('ecore.plic.ploc')
        def do_stuff():
            blahblah()

        with mute_logger('ecore.foo.bar'):
            do_suff()
