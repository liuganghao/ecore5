...
##############################################################################
#
#    eCore, Open Source Business Applications
#    Copyright (C) 2004-2012 eCore S.A. (<http://ecore.com>).
#
...
class KnowledgeConfigSettings(models.TransientModel):
    ...
    module_cmis_read = fields.Boolean(
        'Attach files from an external DMS into eCore',
        help='Connect eCore with a CMIS compatible server to attach files\n'
        'to an eCore record.\n'
        '- This installs the module cmis_read.'
    )

    module_cmis_write = fields.Boolean(
        'Store attachments in an external DMS instead of the eCore Filestore',
        help='Connect eCore with a CMIS compatible server to store files.\n'
