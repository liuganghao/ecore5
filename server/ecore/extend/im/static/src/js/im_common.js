...
/*
    ...
    - load the im_common.xml qweb template into ecore.qweb
    - implement all the stuff defined later
*/

(function() {

function declare($, _, ecore) {
    /* jshint es3: true */
    "use strict";
    ...
    */
    ...
    im_common.ImUser = ecore.Class.extend(ecore.PropertiesMixin, {
        init: function(parent, user_rec) {
            ecore.PropertiesMixin.init.call(this, parent);
        ...
        destroy: function() {
            ...
            ecore.PropertiesMixin.destroy.call(this);
        },
    ...
    im_common.ConversationManager = ecore.Class.extend(ecore.PropertiesMixin, {
        ...
        messages_change: function() {
            if (! ecore.webclient || !ecore.webclient.set_title_part)
                return;
            ecore.webclient.set_title_part("im_messages", this.get("waiting_messages") === 0 ? undefined :
    ...
    im_common.Conversation = ecore.Widget.extend({
        className: "ecore_style oe_im_chatview",
        events: {
        ...
        start: function() {
            ...
            self.$().append(ecore.qweb.render("im_common.conversation", {widget: self}));
            this.$().hide();

            var change_status = function() {
                var disconnected = _.every(this.get("users"), function(u) { return u.get("im_status") === false; });
                self.set("disconnected", disconnected);
                this.$(".oe_im_chatview_users").html(ecore.qweb.render("im_common.conversation.header",
        ...
        received_message: function(message) {
            ...
            this.c_manager.ensure_users([message.from_id[0]]).then(_.bind(function(users) {
                ...
                this._add_bubble(user, message.message, ecore.str_to_datetime(message.date));
            }, this));
        },
        keydown: function(e) {
            if(e && e.which !== 13) {
        ...
        _add_bubble: function(user, item, date) {
            ...
            this.last_bubble = $(ecore.qweb.render("im_common.conversation_bubble", {"items": to_show, "user": user, "time": date}));
...
if (typeof(define) !== "undefined") {
    define(["jquery", "underscore", "ecore"], declare);
} else {
    window.im_common = declare($, _, ecore);
}
