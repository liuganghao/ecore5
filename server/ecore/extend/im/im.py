...
import ecore
import ecore.tools.config
import ecore.modules.registry
from ecore import http
from ecore.http import request
from ecore.osv import osv, fields, expression
from ecore.tools.misc import DEFAULT_SERVER_DATETIME_FORMAT
...
class LongPollingController(http.Controller):
    ...
    def poll(self, last=None, users_watch=None, db=None, uid=None, password=None, uuid=None):
        assert_uuid(uuid)
        if not ecore.evented:
            raise Exception("Not usable in a server not running gevent")
        from ecore.addons.im.watcher import ImWatcher
        if db is not None:
            ecore.service.security.check(db, uid, password)
        else:
...
class im_message(osv.osv):
    ...
    def get_messages(self, cr, uid, last=None, users_watch=None, uuid=None, context=None):
        ...
        c_user = users.browse(cr, ecore.SUPERUSER_ID, my_id, context=context)
        if last:
            if c_user.im_last_received < last:
                users.write(cr, ecore.SUPERUSER_ID, my_id, {'im_last_received': last}, context=context)
        else:
            last = c_user.im_last_received or -1

        # how fun it is to always need to reorder results from read
        mess_ids = self.search(cr, ecore.SUPERUSER_ID, ["&", ['id', '>', last], "|", ['from_id', '=', my_id], ['to_id', 'in', [my_id]]], order="id", context=context)
        mess = self.read(cr, ecore.SUPERUSER_ID, mess_ids, ["id", "message", "from_id", "session_id", "date", "technical"], context=context)
        index = {}
        ...
        if len(mess) > 0:
            last = mess[-1]["id"]
        users_status = users.read(cr, ecore.SUPERUSER_ID, users_watch, ["im_status"], context=context)
    ...
    def post(self, cr, uid, message, to_session_id, technical=False, uuid=None, context=None):
        ...
        self.create(cr, ecore.SUPERUSER_ID, {"message": message, 'from_id': my_id,
...
class im_session(osv.osv):
    ...
    def session_get(self, cr, uid, users_to, uuid=None, context=None):
        ...
        sids = self.search(cr, ecore.SUPERUSER_ID, domain, context=context, limit=1)
        session_id = None
        for session in self.browse(cr, uid, sids, context=context):
            if len(session.user_ids) == len(users):
                ...
                break
        if not session_id:
            session_id = self.create(cr, ecore.SUPERUSER_ID, {
                'user_ids': [(6, 0, users)]
            }, context=context)
        return self.read(cr, uid, session_id, context=context)

    def get_session_users(self, cr, uid, session_id, context=None):
        return self.read(cr, ecore.SUPERUSER_ID, session_id, ['user_ids'], context=context)
    ...
    def remove_me_from_session(self, cr, uid, session_id, uuid=None, context=None):
        my_id = self.pool.get("im.user").get_my_id(cr, uid, uuid, context=context)
        self.write(cr, ecore.SUPERUSER_ID, session_id, {"user_ids": [(3, my_id)]}, context=context)
...
class im_user(osv.osv):
    ...
    def _im_status(self, cr, uid, ids, something, something_else, context=None):
        ...
        data = self.read(cr, ecore.SUPERUSER_ID, ids, ["im_last_status_update", "im_last_status"], context=context)
        for obj in data:
    ...
    def search_users(self, cr, uid, text_search, fields, limit, context=None):
        ...
        users = self.read(cr,ecore.SUPERUSER_ID, found, ["name", "user_id", "uuid", "im_status"], context=context)
    ...
    def _im_change_status(self, cr, uid, new_one, uuid=None, context=None):
        ...
        current_status = self.read(cr, ecore.SUPERUSER_ID, id, ["im_status"], context=None)["im_status"]
        self.write(cr, ecore.SUPERUSER_ID, id, {"im_last_status": new_one, 
    ...
    def get_my_id(self, cr, uid, uuid=None, context=None):
        assert_uuid(uuid)
        if uuid:
            users = self.search(cr, ecore.SUPERUSER_ID, [["uuid", "=", uuid]], context=None)
        else:
            users = self.search(cr, ecore.SUPERUSER_ID, [["user_id", "=", uid]], context=None)
        my_id = users[0] if len(users) >= 1 else False
        if not my_id:
            my_id = self.create(cr, ecore.SUPERUSER_ID, {"user_id": uid if not uuid else False, "uuid": uuid if uuid else False}, context=context)
        return my_id

    def assign_name(self, cr, uid, uuid, name, context=None):
        assert_uuid(uuid)
        id = self.get_my_id(cr, uid, uuid, context=context)
        self.write(cr, ecore.SUPERUSER_ID, id, {"assigned_name": name}, context=context)
        return True
    ...
    def get_users(self, cr, uid, ids, context=None):
        return self.read(cr,ecore.SUPERUSER_ID, ids, ["name", "im_status", "uuid"], context=context)
    ...
    _sql_constraints = [
        ('user_uniq', 'unique (user_id)', 'Only one chat user per eCore user.'),
