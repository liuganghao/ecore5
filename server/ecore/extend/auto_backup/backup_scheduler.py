...
class db_backup(osv.Model):
    ...
    _columns = {
                    ...
                    'sftppath': fields.char('Path external server', help="The location to the folder where the dumps should be written to. For example /ecore/backups/.\nFiles will then be written to /ecore/backups/ on your remote server."),
...
password=passwordLogin, port=portHost)
                    ...
                    #Move to the correct directory on external server. If the user made a typo in his path with multiple slashes (/ecore//backups/) it will be fixed by this regex.
                    ...
                    for file in srv.listdir(pathToWriteTo):
                        ...
                        #If the file is older than the daystokeepsftp (the days to keep that the user filled in on the eCore form it will be removed.
