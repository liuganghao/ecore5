from ecore.addons.saas_base.tools import get_size
import time
import ecore
from ecore import api, models, fields, SUPERUSER_ID, exceptions
...
class SaasServerClient(models.Model):
    ...
    def create_database(self, template_db=None, demo=False, lang='en_US'):
        new_db = self.name
        if template_db:
            ecore.service.db._drop_conn(self.env.cr, template_db)
            ecore.service.db.exp_duplicate_database(template_db, new_db)
        else:
            password = ''.join(random.SystemRandom().choice(string.ascii_uppercase + string.digits) for _ in range(32))
            ecore.service.db.exp_create_database(new_db, demo, lang, user_password=password)
        self.state = 'open'

    @api.one
    def registry(self, new=False, **kwargs):
        m = ecore.modules.registry.RegistryManager
        if new:
    ...
    def _get_data(self, client_env, check_client_id):
        ...
        users_len = len(users)
        data_dir = ecore.tools.config['data_dir']
    ...
    def delete_database(self):
        ecore.service.db.exp_drop(self.name)
        self.write({'state': 'deleted'})

    @api.one
    def rename_database(self, new_dbname):
        ecore.service.db.exp_rename(self.name, new_dbname)
