{
    ...
    'description': '''
Addon allows to users to login in ecore databases via master database
...
* User -- a human
* Master -- an ecore database where this module is installed
* Client -- an ecore database where you need to authenticate user and where module auth_oauth module is installed
...
From client database redirects user to url1 at master database:
    ...
    'redirect_uri': 'http://ecore.example.com/auth_oauth/signin',
...
then master redirects user back:
    ...
    http://ecore.example.com/auth_oauth/signin#access_token=ksESez4jRUxg0v6Kt7HkE5Z4ZtyrrM&token_type=Bearer&state=%7B%27p%27%3A+1%2C+%27r%27%3A+%27%252Fweb%252Flogin%253F%27%2C+%27d%27%3A+%27some-test-3%27%7D&expires_in=3600&scope=userinfo
