...
import ecore
from ecore.service.server import memory_info
from ecore.service import security, model as service_model
from ecore.tools.func import lazy_property
from ecore.tools import ustr, consteq
...
# *broken*
NO_POSTMORTEM = (ecore.osv.orm.except_orm,
                 ecore.exceptions.AccessError,
                 ecore.exceptions.ValidationError,
                 ecore.exceptions.MissingError,
                 ecore.exceptions.AccessDenied,
                 ecore.exceptions.Warning,
                 ecore.exceptions.RedirectWarning)
    ...
    try:
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_request and rpc_response_flag:
                ecore.netsvc.log(rpc_request, logging.DEBUG, '%s.%s' % (service_name, method), replace_request_password(params))

        threading.current_thread().uid = None
        threading.current_thread().dbname = None
        if service_name == 'common':
            dispatch = ecore.service.common.dispatch
        elif service_name == 'db':
            dispatch = ecore.service.db.dispatch
        elif service_name == 'object':
            dispatch = ecore.service.model.dispatch
        elif service_name == 'report':
            dispatch = ecore.service.report.dispatch
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_response_flag:
                ecore.netsvc.log(rpc_response, logging.DEBUG, logline, result)
            else:
                ecore.netsvc.log(rpc_request, logging.DEBUG, logline, replace_request_password(params), depth=1)

        return result
    except NO_POSTMORTEM:
        raise
    except ecore.exceptions.DeferredException, e:
        _logger.exception(ecore.tools.exception_to_unicode(e))
        ecore.tools.debugger.post_mortem(ecore.tools.config, e.traceback)
        raise
    except Exception, e:
        _logger.exception(ecore.tools.exception_to_unicode(e))
        ecore.tools.debugger.post_mortem(ecore.tools.config, sys.exc_info())
        raise
...
class WebRequest(object):
    """ Parent class for all eCore Web request types, mostly deals with
    ...
    def __init__(self, httprequest):
        ...
        # dispatching phase in ecore.service.wsgi_server.application
        if self.db:
    ...
    def session(self):
        """
        a :class:`eCoreSession` holding the HTTP session data for the
    ...
    def cr(self):
        """
        :class:`~ecore.sql_db.Cursor` initialized for the current method
        call.
    ...
    def _handle_exception(self, exception):
        ...
        if not isinstance(exception, NO_POSTMORTEM) \
            ...
            ecore.tools.debugger.post_mortem(
                ecore.tools.config, sys.exc_info())
        raise
    ...
    def session_id(self):
        """
        opaque identifier for the :class:`eCoreSession` instance of
        the current request
    ...
    def registry(self):
        ...
        .. deprecated:: 8.0
            ...
            use :attr:`.env`
        """
        return ecore.modules.registry.RegistryManager.get(self.db) if self.db else None
...
def route(route=None, **kw):
    ...
    .. admonition:: CSRF Protection
        ...
        eCore implements token-based `CSRF protection
        ...
        * if the form is generated in Python, a csrf token is
          ...
          <ecore.http.WebRequest.csrf_token`, the
          :data:`~ecore.http.request` object is available by default
        ...
        * if the endpoint can be called by external parties (not from
          eCore) as e.g. it is a REST API or a `webhook
    ...
    * the handler method's result is returned as JSON-RPC ``result`` and
    ...
    def _handle_exception(self, exception):
        ...
        try:
            ...
            if not isinstance(exception, (ecore.exceptions.Warning, SessionExpiredException, ecore.exceptions.except_orm)):
                _logger.exception("Exception during JSON request handling.")
            error = {
                    'code': 200,
                    'message': "eCore Server Error",
                    'data': serialize_exception(exception)
            }
            if isinstance(exception, AuthenticationError):
                error['code'] = 100
                error['message'] = "eCore Session Invalid"
            if isinstance(exception, SessionExpiredException):
                error['code'] = 100
                error['message'] = "eCore Session Expired"
...
def serialize_exception(e):
    ...
    elif isinstance(e, ecore.exceptions.Warning):
    ...
    elif isinstance(e, ecore.exceptions.ValidationError):
        tmp["exception_type"] = "validation_error"
    elif isinstance(e, ecore.exceptions.except_orm):
...
def jsonrequest(f):
    """ 
        .. deprecated:: 8.0
            Use the :func:`~ecore.http.route` decorator instead.
    """
...
eCore URLs are CSRF-protected by default (when accessed with unsafe
HTTP methods). See
https://www.ecore.com/documentation/9.0/reference/http.html#csrf for
more details.

* if this endpoint is accessed through eCore via py-QWeb form, embed a CSRF
...
class ControllerType(type):
    def __init__(cls, name, bases, attrs):
        ...
        if not class_path[:2] == ["ecore", "addons"]:
            module = ""
        else:
...
def routing_map(modules, nodb_only, converters=None):
    ...
    def get_subclasses(klass):
        def valid(c):
            return c.__module__.startswith('ecore.addons.') and c.__module__.split(".")[2] in modules
...
class eCoreSession(werkzeug.contrib.sessions.Session):
    def __init__(self, *args, **kwargs):
        self.inited = False
        self.modified = False
        self.rotate = False
        super(eCoreSession, self).__init__(*args, **kwargs)
        self.inited = True
    ...
    def _fix_lang(self, context):
        """ eCore provides languages which may not make sense and/or may not
    ...
    def assert_valid(self, force=False):
        ...
        Ensures this session is valid (logged into the ecore server)
        """
    ...
    def model(self, model):
        ...
        :param model: an eCore model name
        :type model: str
...
class Response(werkzeug.wrappers.Response):
    ...
    def render(self):
        ...
        uid = self.uid or request.uid or ecore.SUPERUSER_ID
...
class Root(object):
    """Root WSGI application for the eCore Web Client.
    """
    def __init__(self):
        self._loaded = False
    ...
    def session_store(self):
        # Setup http sessions
        path = ecore.tools.config.session_dir
        _logger.debug('HTTP sessions stored in: %s', path)
        return werkzeug.contrib.sessions.FilesystemSessionStore(path, session_class=eCoreSession)

    @lazy_property
    def nodb_routing_map(self):
        _logger.info("Generating nondb routing")
        return routing_map([''] + ecore.conf.server_wide_modules, True)
    ...
    def load_addons(self):
        ...
        for addons_path in ecore.modules.module.ad_paths:
            for module in sorted(os.listdir(str(addons_path))):
                if module not in addons_module:
                    manifest_path = os.path.join(addons_path, module, '__ecore__.py')
                    ...
                    if os.path.isfile(manifest_path) and os.path.isdir(path_static):
                        ...
                        if 'ecore.addons' in sys.modules:
                            m = __import__('ecore.addons.' + module)
                        else:
                            m = None
    ...
    def setup_session(self, httprequest):
        ...
        if not sid:
            sid =  httprequest.headers.get("X-Ecore-Session-Id")
        if not sid:
    ...
    def dispatch(self, environ, start_response):
        ...
        try:
            ...
            with request:
                ...
                if db:
                    ecore.modules.registry.RegistryManager.check_registry_signaling(db)
                    try:
                        with ecore.tools.mute_logger('ecore.sql_db'):
                        ...
                        ecore.modules.registry.RegistryManager.signal_caches_change(db)
                else:
                    result = _dispatch_nodb()
...
def db_list(force=False, httprequest=None):
    dbs = ecore.service.db.list_dbs(force)
...
def db_filter(dbs, httprequest=None):
    ...
    if d == "www" and r:
        d = r.partition('.')[0]
    r = ecore.tools.config['dbfilter'].replace('%h', h).replace('%d', d)
...
class CommonController(Controller):
    ...
    def jsonrpc(self, service, method, args):
        """ Method used by client APIs to contact eCore. """
