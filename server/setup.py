...
execfile(join(dirname(__file__), 'ecore', 'release.py'))  # Load release variables
lib_name = 'ecore'
...
def py2exe_datafiles():
    ...
    for root, dirnames, filenames in os.walk('ecore'):
    ...
    if os.name == 'nt':
        ...
        return {
            'console': [
                {'script': 'ecore.py'},
                {'script': 'ecore-gevent'},
                {'script': 'ecore-server', 'icon_resources': [
                    (1, join('setup', 'win32', 'static', 'pixmaps', 'ecore-icon.ico'))
                ]},
            ],
            'options': {
                'py2exe': {
...
setup(
    ...
    license=license,
    scripts=['ecore-server', 'ecore.py'],
    packages=find_packages(),
    package_dir={'%s' % lib_name: 'ecore'},
